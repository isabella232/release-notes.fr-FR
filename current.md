...
* [Notes de mise à jour de la collecte de données d’Experience Platform](https://experienceleague.adobe.com/docs/launch/using/release-notes/current.html?lang=fr) (anciennement, Experience Platform Launch) - Mis à jour le **22 avril 2021**
...
| Mises à jour des zones géographiques ISO 2021 | 13 mai 2021 | Adobe effectuera les mises à jour de région ISO 2021 le 21 mai 2021. Les mises à jour mineures qui suivront cette version sont attendues. |
...
| -----------| ---------- | ---------- | ---------- |
| Mai 2021 | [Définition de filtres d’URL internes](https://experienceleague.adobe.com/docs/analytics-learn/tutorials/administration/manage-report-suites/set-internal-url-filters.html?lang=fr) | Vidéo | Découvrez les paramètres des URL [!UICONTROL Référent], en les définissant comme référents internes (pour alimenter les rapports de cheminement) ou comme référents externes (pour alimenter les rapports sur les domaines référents). |
| Mai 2021 | [Configuration des méthodes de recherche](https://experienceleague.adobe.com/docs/analytics-learn/tutorials/administration/manage-report-suites/configure-finding-methods.html?lang=fr) | Vidéo | Découvrez comment configurer les paramètres qui contrôlent la manière dont les moteurs de recherche, les mots-clés et les autres rapports et composants de méthode de recherche traitent l’attribution des événements de conversion. |
...
| Mai 2021 | [Formatage des mesures dans les vues de données](https://experienceleague.adobe.com/docs/customer-journey-analytics-learn/tutorials/formatting-metrics-in-data-views.html?lang=en) | Vidéo | Découvrez les options de formatage des mesures dans l’écran des composants de la configuration [!UICONTROL Vue de données]. Cette vidéo présente également quelques conseils qui peuvent vous aider à tirer le meilleur parti de vos composants. |
...
* **Adobe Experience Manager as a Cloud Service**
   ...
   * **Experience Manager Sites as a Cloud Service**
      ...
      * Points de terminaison GraphQL : vous pouvez désormais activer l’API Adobe Experience Manager GraphQL pour les configurations des sites individuels d’Experience Manager et créer des points de terminaison GraphQL personnalisés pour ces configurations à l’aide d’une nouvelle interface utilisateur de la console GraphQL. L’interface utilisateur permet également de gérer les points de terminaison GraphQL.
   ...
   * **Experience Manager Forms as a Cloud Service**
      ...
      * **Prise en charge de l’utilisation de l’option littérale pour définir la valeur d’une variable de type JSON**
         ...
         Vous pouvez utiliser l’option littérale pour définir la valeur d’une variable de type JSON à l’étape de définition de la variable d’un flux de travail Experience Manager. L’option littérale vous permet de spécifier un fichier JSON dans le formulaire d’une chaîne.
   ...
   * **Experience Manager Commerce as a Cloud Service**
      ...
      * Prise en charge de l’UID de catégorie - cette fonctionnalité débloque des intégrations commerciales tierces pour les systèmes qui utilisent des chaînes pour les identifications de catégorie.
   ...
   * **Cloud Manager**
      ...
      * Le point de terminaison du référentiel dans l’API Public inclut désormais l’URL Git.
      ...
      * Le module complémentaire Commerce peut désormais être appliqué aux programmes Sandbox pendant le flux de travail Modifier le programme.
      ...
      * L’onglet Solutions des flux de travail Ajouter un programme et Modifier le programme affiche la solution, même si une seule solution est disponible pour le programme.
      * Le message d’erreur à l’étape du journal de génération lorsque la version ne produisait aucun module de contenu déployé n’était pas clair.
...
* **[Adobe Summit 2021 Sneaks avec Dan Levy](Https://experienceleaguecommunities.adobe.com/t5/adobe-experience-manager/adobe-summit-2021-sneaks-with-dan-levy/td-p/405865)**
...
* **[Les communautés d’Experience League lancent la ludification](https://experienceleaguecommunities.adobe.com/t5/adobe-experience-manager/experience-league-communities-launches-the-gamification/td-p/405366)**
...
* **[Adobe Summit 2021 | Liste complète des sessions Experience Manager](https://experienceleaguecommunities.adobe.com/t5/adobe-experience-manager/adobe-summit-2021-complete-aem-session-list/td-p/398344)**
   ...
   Suite à de nombreuses requêtes de clients Adobe, vous trouverez sur cette page la liste complète des sessions Experience Manager qui auront lieu lors de l’événement **Adobe Summit 2021**.

* **[Annonce d’un nouveau processus d’envoi de demandes de fonctionnalités Experience Manager](https://experienceleaguecommunities.adobe.com/t5/adobe-experience-manager/announcing-the-new-process-to-submit-experience-manager-feature/td-p/380425)**
...
| Mai 2021 | [Configuration rapide de l’éditeur SPA et SPA distante](https://experienceleague.adobe.com/docs/experience-manager-learn/getting-started-with-aem-headless/spa-editor/remote-spa/quick-setup.html?lang=fr) | Article | La configuration rapide est une présentation accélérée qui illustre comment installer et exécuter l’application WKND en tant que SPA distante, et la créer à l’aide de l’éditeur AEM SPA. |
| Mai 2021 | [Formulaire adaptatif avec Adobe Sign](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/forms/forms-and-sign/create-adaptive-form.html?lang=fr) | Vidéo | Découvrez comment créer un formulaire adaptatif pour utiliser la configuration cloud Adobe Sign. |
| Mai 2021 | [Configuration d’un formulaire adaptatif pour le scénario « remplir et signer »](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/forms/forms-and-sign/configure-form-fill-and-sign.html?lang=fr) | Vidéo | Découvrez comment configurer le formulaire lorsque l’utilisateur et le signataire sont la même personne. |
| Mai 2021 | [Création d’une application API Adobe Sign](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/forms/forms-and-sign/create-sign-api-application.html?lang=fr#forms) | Vidéo | Découvrez comment créer une application API sur votre Adobe Sign pour l’intégrer à votre plateforme Adobe Sign. |
...
| -----------| ---------- | ---------- | ---------- |---------- |
| 27 avril 2021 | [Intégrer Campaign Standard à Analytics pour optimiser le marketing par e-mail](https://experienceleague.adobe.com/?lang=fr#dashboard/learning) | Campaign Standard, Analytics | Cours | Découvrez comment intégrer Campaign Standard à Adobe Analytics et optimiser vos stratégies de marketing par e-mail à l’aide des données en temps réel. Ce cours explique comment créer un rapport Campaign Standard dans Adobe Analytics. Ensuite, découvrez comment utiliser les [!UICONTROL déclencheurs] Experience Cloud et [!UICONTROL Platform Launch] pour configurer les messages marketing et transactionnels en fonction de l’activité du client. |
...
| 12 avril 2021 | [Configuration de messages transactionnels à l’aide de données d’événement en temps réel](https://experienceleague.adobe.com/docs/campaign-standard-learn/tutorials/integrations/triggers/configure-transactional-messages-using-realtime-event-data.html?lang=fr) | Campaign Standard | Vidéo | Découvrez comment configurer des messages transactionnels à l’aide de données d’événement en temps réel qui proviennent de [!UICONTROL déclencheurs]. |
...
| 12 avril 2021 | [Utilisation de déclencheurs pour les messages transactionnels](https://experienceleague.adobe.com/docs/campaign-standard-learn/tutorials/integrations/triggers/using-triggers-for-transactional-messaging-overview.html?lang=fr#integrations) | Campaign Standard | Vidéo | Découvrez comment utiliser des [!UICONTROL déclencheurs] pour les messages transactionnels. |
...
| -----------| ---------- |
| Paramètres du module | Une nouvelle option [!UICONTROL Stratégie de régulation du remplissage], « [!UICONTROL légèrement en avant ]», est disponible. Il s’agit de la valeur par défaut des nouveaux modules. Cette stratégie permet d’accélérer la diffusion de sorte qu’elle soit achevée à 55-65 % au milieu de la durée du vol. |
...
Voir les [notes de mise à jour](https://experienceleague.adobe.com/docs/document-cloud-learn/sign-learning-hub/integrations/microsoft/documentautomation.html#integrations) [!DNL Marketo Engage] pour consulter les informations sur la planification de la dernière version.
