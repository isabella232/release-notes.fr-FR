...
* Ajout de ` idSyncAttachIframeOnWindowLoad` comme indicateur booléen en option que vous pouvez définir dans la fonction ` Visitor.getInstance`. Si ` idSyncAttachIframeOnWindowLoad= true`, le service d’identification charge l’iFrame de synchronisation des identifiants au chargement de la fenêtre. Par défaut, le service d’identification charge l’iFrame aussi vite que possible. Cet indicateur *remplace*` idSyncAttachIframeASAP`, qui est obsolète. Pour plus d&#39;informations, voir [
Variables utilisées par la fonction get Service du service d&#39;identification](https://marketing.adobe.com/resources/help/en_US/mcvid/?f=mcvid-variables.html). (MCID-122)
* Ajout d’une fonctionnalité permettant la prise en charge du suivi des [!DNL  Marketing Cloud] ID à l’échelle de tous les domaines, applications natives et applications hybrides jusqu’aux transitions web. Pour plus d&#39;informations, voir [Ajout de la fonction d’assistance d’identification du visiteur](https://marketing.adobe.com/resources/help/en_US/mcvid/mcvid-appendvisitorid.html). (MCID-42)
* Ajout de fonctions au code visitorAPI.js qui déterminent si le service d’ID a généré l’[!DNL  Marketing Cloud] ID du visiteur côté client ou côté serveur ou si les appels d’identifiants sont expirés. Pour plus d&#39;informations, voir [Fonctions de suivi de délai d’attente](https://marketing.adobe.com/resources/help/en_US/mcvid/?f=mcvid-timeout-functions.html) et [Suivi de la génération d’identifiant de visiteur côté client](https://marketing.adobe.com/resources/help/en_US/mcvid/?f=mcvid-client-side-id.html). (MCID-116)
...
* Prise en charge du code DIL pour le partage des ressources Cross-Origin (CORS). Pour plus d&#39;informations, voir [useCorsOnly](https://marketing.adobe.com/resources/help/en_US/aam/?f=dil-use-cors-only.html). (AAM-25319, AAM-20139).
* L’iFrame DIL prend en charge les attributs ` iframe.sandbox='allow-scripts allow-same-origin';`. Le fait d’autoriser seulement ces deux jetons renforce la sécurité. Pour plus d’informations, reportez-vous à la section Attributs de cette [documentation iFrame](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/iframe). (AAM-23335)
* Le code DIL peut maintenant émettre des appels HTTPS sécurisés par défaut. Les clients qui utilisent le code DIL [!DNL  Audience Manager] et le code visitorapi.js sur la même page doivent définir la variable DIL ` secureDataCollection= false`. Pour plus d&#39;informations, voir [secureDataCollection](https://marketing.adobe.com/resources/help/en_US/aam/?f=dil-secure-data-collection.html).
