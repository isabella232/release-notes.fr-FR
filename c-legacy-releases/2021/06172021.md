...
| [!UICONTROL Centre de notifications Beta] | Vous pouvez désormais recevoir et gérer des notifications en dehors de votre navigateur en installant une application [!UICONTROL Notification Center Beta] pour Google Chrome 40 et versions ultérieures ou Microsoft Edge 17 et versions ultérieures. L’application se charge plus rapidement que la version normale du navigateur et est automatiquement mise à jour.<br><br>Une fois l’application installée, l’application web se charge sous la forme d’une fenêtre distincte qui réorganise dynamiquement sa mise en page en fonction de la taille de la fenêtre. Vous n’avez pas besoin de vous connecter à l’application ni de disposer d’une fenêtre de navigateur ouverte pour l’utiliser. |
|  | Un nouveau type d’alerte, [!UICONTROL Actions de l’interface utilisateur de gestion de campagne], est disponible et affiche l’état final de toutes les tâches en arrière-plan, y compris les modifications en masse et les modifications d’affectation des portefeuilles, des contraintes, des classifications et des événements. Vous êtes automatiquement abonné aux alertes pour les actions de l’interface utilisateur.<br><br>Ce type d’alerte remplacera éventuellement les notifications d’état de la tâche héritée disponibles dans la barre d’outils appropriée (notifications d’état de la tâche héritée). |
| Rapports | Le nouveau [!UICONTROL rapport Heure de la campagne] affiche les données de coût, de clic et (éventuellement) de conversion pour une ou plusieurs campagnes Google avec des mots-clés pour chaque heure au cours de la période spécifiée. Ce rapport est identique au [!UICONTROL rapport Campagne] avec une agrégation des données de &quot;Heure&quot; ou &quot;Jour de la semaine (Heure)&quot;. Ces types d’agrégation seront abandonnés du [!UICONTROL rapport de campagne] dans une version ultérieure. |
