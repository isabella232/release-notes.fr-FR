...
<table id="table_EB3FFBA2DF904546A5185EC9A63BBA98"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Lorsque vous liez des comptes de solution dans le Marketing Cloud, si vous avez plusieurs solutions et organisations, vous pouvez maintenant mapper plusieurs produits et services à une seule organisation. </p> <p>Voir <a href="https://marketing.adobe.com/resources/help/fr_FR/mcloud/organizations.html" format="https" scope="external">Comptes et organisations </a>. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Ajouté la rubrique <a href="https://marketing.adobe.com/resources/help/fr_FR/mcloud/core_services.html" format="https" scope="external"> Services principaux - Activation de vos solutions </a> pour vous aider à mettre en oeuvre les services principaux. </p> </td> 
  </tr> 
 </tbody> 
</table>

Pour plus d’informations, voir la documentation produit de [Marketing Cloud et services principaux](https://docs.adobe.com/content/help/fr-FR/core-services/interface/about-core-services/core-services-landing.html).
...
* Correction d’un problème en raison duquel les règles copiées n’étaient pas automatiquement ajoutées à la bibliothèque d’évaluation tant qu’une modification n’avait pas été apportée aux règles.
* Correction d’un problème en raison duquel Regex se déclenchait pour les règles utilisant l’option de condition &quot;paramètres&quot;, même si le paramètre n’était pas présent.

Pour obtenir de l’aide ou des informations sur les notes de mise à jour actuelles et cumulatives et sur les mises à jour de la documentation, voir [Nouveautés de la Dynamic Tag Management](https://docs.adobe.com/content/help/en/dtm/using/whatsnew.html).
...
<table id="table_91D1FD20C4C1411292252364328677AF"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> Rapports <span class="wintitle"> Analytics </span> Mises à jour de navigation </td> 
   <td colname="col2"> 
  ...
  <tr> 
   ...
   <td colname="col2"> 
    <ul id="ul_9F4F40B4329443AC88F07B74660407C3"> 
     ...
     <li id="li_8EEB2C69113C43349F2B5E3AD9352B7B">Ajoute la possibilité d’exécuter un ensemble de mesures sans les enregistrer comme mesures par défaut. Cette fonction est désormais plus cohérente avec les rapports Rapports et analyses standard. </li> 
    </ul> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="wintitle"> Règles de  </span> classification Analytics - remplacer les valeurs existantes </p> </td> 
   <td colname="col2"> <p>Dans <span class="uicontrol">Analytics </span> &gt; <span class="uicontrol">Admin </span> &gt; <span class="uicontrol">Créateur de règles de classification </span> &gt; <span class="term">&lt;nom du jeu de règles&gt; </span> , deux nouvelles options permettent de sélectionner un mode de remplacement : </p> 
    <ul id="ul_54AAEB16B74B4F53BE3549A7992B7353"> 
     ...
     <li id="li_6F71992C30E44B6FAC8E7C20D4C9D6DF"> <b>Les règles remplacent uniquement les valeurs non définies :</b> seules les cellules vides (non définies) sont remplies. Les classifications existantes restent inchangées. </li> 
    </ul> <p>Auparavant, si une clé était déjà classée dans une colonne ou une cellule, la règle ne s’exécutait pas sur cette clé et la ligne du tableau était ignorée. </p> <p>Voir la documentation sur le produit <a href="https://marketing.adobe.com/resources/help/en_US/reference/classification_rule_set.html" format="https" scope="external">Règles de classification</a> pour en savoir plus. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> Polarité des mesures </td> 
   <td colname="col2"> <p>Remarque :  </p> <p>1. La polarité des mesures vous permet d’indiquer si l’Adobe <span class="wintitle"> Analytics </span> doit considérer qu’il est bon ou mauvais si un événement personnalisé (mesure) donné augmente. </p> <p>2. Il permettra à l’Adobe <span class="wintitle"> Analytics </span> d’afficher des indicateurs directionnels (flèches) pour diverses mesures afin d’ajouter du contexte (par exemple, comparaisons d’une semaine à l’autre). Exemple : si "Bugs Submitted" augmente d’une semaine à l’autre, l’Adobe <span class="wintitle"> Analytics </span> doit-il considérer que c’est bon ou mauvais ? Autres exemples : une augmentation des inscriptions au publipostage est probablement bénéfique. En revanche, une augmentation des erreurs de soumission de formulaire est probablement mauvais signe. </p> <p>Dans <span class="ignoretag"><span class="uicontrol"> 
...
* Mises à jour d’Instance entre pour les segments séquentiels. A l’origine, cette fonction vous permettait de faire correspondre un événement dans un segment séquentiel, tel que la fin d’une visite, et de définir l’heure dans ou après la logique en fonction de ces points de contrôle de événement. Désormais, une nouvelle fonction de restriction de conteneur vous permet de spécifier une limite et un nombre et d’identifier le conteneur dans la création d’un segment, ce qui vous permet de créer des segments séquentiels plus ciblés et plus complexes.
* Dimension entre les mises à jour pour les segments séquentiels. Tout comme la logique Instance entre qui identifie les correspondances entre les conteneurs, la Dimension entre inclut désormais une fonction de restriction de dimension qui fournit une logique basée sur les valeurs de dimension, en déterminant les correspondances basées sur des valeurs spécifiques (lignes de tableau) pour suivre si une dimension sélectionnée est définie (ou non) et vous permet ensuite de fournir une logique de segment basée sur ces valeurs de dimension.
* La segmentation séquentielle ajoute une correspondance à plusieurs instances. La mise en oeuvre précédente de la logique Instance entre utilisait uniquement la correspondance de première instance pour les segments séquentiels, sans correspondance avec les séquences légitimes trouvées plus tard dans la visite. La nouvelle logique permet désormais de capturer toute séquence correspondante pendant la visite dans le cadre du segment séquentiel. Par exemple, un segment séquentiel capturant des visiteurs qui passaient à *page A puis page B mais pas page C* ne correspondait pas si le visiteur passait à *A à B à C puis revenait à B*, ce qui correspond dans une deuxième instance (revenir à la page B correspond à la logique *A à B et non à C*).
...
>[!NOTE]
>
>l’accès à l’entrepôt de données de la version 14 sera supprimé dans cette version.[En savoir plus](https://docs.adobe.com/content/help/fr-FR/analytics/components/segmentation/seg-home.html)

### Flux de données Clickstream {#clickstream}

Renvois de données pour les flux de données horaires :

Si vous demandez des données pour des dates antérieures lors de la configuration d’un nouveau flux de données horaire, les données relatives aux dates d’il y a plus de 60 jours peuvent être distribuées au format quotidien plutôt qu’horaire.

Dans ce cas, vous ne recevrez pas 24 diffusions distinctes pour ces jours, mais vous recevrez une seule diffusion avec un horodatage de minuit qui contient toutes les données pour cette journée. Si vous demandez ce type de renvoi, assurez-vous que votre ETL est configuré pour traiter les diffusions quotidiennes.

Voir [Configuration des flux de données](https://marketing.adobe.com/resources/help/en_US/sc/clickstream/datafeeds_configure.html) pour plus d’informations.
...
Consultez les [Notes de mise à jour d&#39;Adobe Target](https://docs.adobe.com/content/help/fr-FR/target/using/release-notes/release-notes.html) pour connaître les dernières dates et les informations de mise à jour.
