...
La section [Comprendre la synchronisation des identifiants et les taux de correspondance](https://marketing.adobe.com/resources/help/en_US/mcvid/mcvid-match-rates.html) 
inclut de nouvelles informations sur l’appel ` cm.eversttech.net` que le service d’identification passe à [!DNL  Adobe Media Optimizer].
...
<table id="table_A41D8C79DEE84AE59FE9C8D2D291FCD7"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>Lorsque l’option d’optimisation automatique des ajustements d’offres en fonction de l’emplacement ou de la liste de remarketing est activée, Media Optimizer modifie désormais quotidiennement les ajustements d’offres pertinents pour les campagnes pertinentes du portefeuille optimisé. Ces modifications sont effectuées en fonction des besoins dans le cadre du processus d’optimisation. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"></td> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>(Comptes Google) Le service principal Adobe Mobile Services est désormais intégré parmi les partenaires de mesure sur appareils mobiles (MMP). Un paramètre de niveau d’annonceur vous permet de définir Adobe Mobile Services comme MMP de l’annonceur. </p> <p>Lorsqu’un annonceur utilise le service principal Adobe Mobile Services, les paramètres d’annonce d’installation d’application et d’extension d’application incluent une URL de postback. Une fois par application, entrez manuellement cette URL de postback pour les événements d’installation et les autres événements in-app dans les paramètres de l’application. </p> <p>Le service principal Adobe Mobile Services envoie ensuite des données à Media Optimizer pour chaque installation ou tout autre événement in-app en envoyant une requête ping à l’URL de postback. Lorsque Media Optimizer reçoit les requêtes de postback, les événements suivis sont disponibles sous forme de propriétés de transaction. Pour inclure les propriétés de transaction dans vos vues et rapports, ou pour configurer vos objectifs de portefeuille de façon à les optimiser pour des propriétés de transaction spécifiques, vous devez permettre aux portefeuilles, rapports et vues d’y accéder. </p> </td> 
  </tr> 
  <tr> 
